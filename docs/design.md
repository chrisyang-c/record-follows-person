...
- **衍生 tokens（2026-09-05，記於 DECISIONS.md）**：§7 的原色只用於邊框、填色、圖示；放在填色或白底上的文字改用衍生色，因為 §7 的 `--ok` 白底對比只有 3.0:1（`--warn` 2.5:1、`--danger` 4.4:1），低於 §7 自己的 ≥4.5:1 正文規則。
  - `--ok-fill #E8F7F0` / `--warn-fill #FFF4E0` / `--danger-fill #FBE9E9`：chip、確認鍵、紅燈 banner 的填色（夜班：`#143528` / `#3D2D0F` / `#4A1F1F`）。
  - `--ok-ink #14724D` / `--warn-ink #8A5700` / `--danger-ink #A83232`：填色上或白底上的文字（夜班：`#7FD8B0` / `#F0B95A` / `#F28B8B`）。
  - `--primary-hover #1A62D3` / `--surface-hover #EEF2F7` / `--ok-fill-hover #D6F0E4` / `--danger-fill-hover #F5D5D5`：hover「加深 6%」的落點，程式裡不再出現 hex。

## 2. 三個介面的硬規格
| 介面 | 裝置 | 規格 |
|---|---|---|
| 照護者 | 手機（390px 優先，桌機置中同寬） | LINE 式聊天：氣泡（系統 `--surface`、照護者 `--primary` 白字）、底部固定輸入列（麥克風＋文字＋送出，皆 ≥56px，拇指可及，`env(safe-area-inset-bottom)`）；沒有頭像、沒有「AI 思考中」、沒有星星；一次只問一題（由模型決定），照護者用語音或文字回答，沒有快速回覆按鈕；「跟平常不一樣」與四個事件快捷是第一則訊息下方的按鈕列；結束出「我理解的是這樣」摘要卡（照護者口吻）；文案口語（「王伯今天吃得怎樣？」），不用表單語；zh-TW（多語第二階段）。 |
