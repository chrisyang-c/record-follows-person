...
| 介面 | 裝置 | 規格 |
|---|---|---|
| 照護者 | 手機（390px 優先，桌機置中同寬） | LINE 式聊天：氣泡（系統 `--surface`、照護者 `--primary` 白字）、底部固定輸入列（麥克風＋文字＋送出，皆 ≥56px，拇指可及，`env(safe-area-inset-bottom)`）；沒有頭像、沒有「AI 思考中」、沒有星星；一次只問一題，每題 2–4 個快速回覆＋「不知道」（≥56px）；「跟平常不一樣」與四個事件快捷是第一則訊息下方的按鈕列；結束出「我理解的是這樣」摘要卡（照護者口吻）；文案口語（「王伯今天吃得怎樣？」），不用表單語；zh-TW（多語第二階段）。 |
