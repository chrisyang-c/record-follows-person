...
| 26 | gpt-5.6-luna（reasoning_effort=none）比 gpt-4.1 更常無視「已問過」而逐字重問同一題（實測連問三次「叫得醒嗎」）。現在 `intake_dialog._plan` 驗證模型的決定：重複的問題或已知維度 → 帶原因重問一次，第二次仍無效 → 503 顯示錯誤（不退回規則版）；模型回傳維度中文名也接受。 | 偶爾一輪多一次 planner 呼叫（約 +$0.0002）。 | 換回 gpt-4.1 或提高 reasoning_effort（後者需改用 Responses API 才能用 function tools）。 |
