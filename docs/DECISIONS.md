...
> **給初學者：** 這是一份「為什麼要這樣做」的歷史筆記，不是操作手冊。先看 [入門指南](BEGINNER_GUIDE.md) 了解系統，再用本頁追查某個架構決定的日期、理由與影響。
...
| 2026-09-05 | 01 人體圖改用 EMBL-EBI anatomogram 的向量解剖 SVG（Apache-2.0，76 個 UBERON 器官群組），以 CSS 點亮成青色 X 光樣、選中維度器官改紫色；熱點位置由器官外框即時算出。手畫線稿刪除。試過 Wikimedia「Man shadow anatomy」但內嵌的是低解析點陣圖層。 | 使用者要求「精密、不要假的」；向量器官可依維度點亮且能列印。 | Claude |
| 2026-09-05 | 新增 `apps/api/baseline/`：從 timeline 已量測的 vitals 算出每位住民自己的正常帶（中位數＋MAD＋p10–p90），`established` 門檻（≥12 筆、涵蓋 ≥5 天、變異>0）未達就完全不產生判斷。新增紅燈 RF13「偏離他自己平常的量測範圍」，action=`observe` 不是 `notify_now`。不寫 baseline：`propose_vitals_usual` 只產提案，寫入仍走 ◇nurse_confirm_baseline。 | RF02–RF04 是族群門檻，回答「這個值對任何人危不危險」；P001 平常收縮壓 138，一個 118 通過所有族群門檻卻是真的掉了 20 點。實測：若改用族群門檻 <125 想抓到他，P002（平常 126）90 天內會響 63/180 次（35%）——族群門檻只有太鈍或太吵兩種結局。RF13 排 observe 是因為偏離個人基準是早期訊號，放進紅燈橫幅會淹掉真正要立刻叫人的規則。 | Claude |
| 2026-09-05 | `record/store.py` 的五處 `read_text()` 補上 `encoding="utf-8"`。 | Windows 的 `read_text()` 預設用平台編碼（繁中 Windows 是 cp950），讀 UTF-8 的 JSON 直接 UnicodeDecodeError；在 Windows 上 `uv run pytest` 有 25 個測試因此失敗，與程式邏輯無關。補上後 133 個測試全過。 | Claude |
| 2026-09-05 | 移除 `baseline/vitals_band.py` 的 `propose_vitals_usual()`（上一個 commit 才加的），並加一個測試確保它不會被加回來。個人正常帶只回答「這次量到的跟他實際被量到的比起來如何」，不提供任何寫回 `vitals_usual` 的路徑。 | ARCHITECTURE §11：「baseline 只在醫囑或護理師確認時更新，不自動漂移，否則『平常』會被慢慢惡化帶走。」對一個逐漸衰退的人，每一次「你的量測顯示他的平常變了」提案在當下都合理，護理師按幾次之後『平常』就跟著病情走，偏離偵測從此永遠不會再命中。`baseline_update_proposal` 只從醫囑產生提案，這個設計是對的，不該從量測開第二個入口。 | Claude |
| 2026-09-05 | 工作區整併：`record-follows-person` 定為唯一正式專案。`docs/CONSOLIDATION.md` 記錄每份來源的去向；外層 `00-architecture.md` 收進 `docs/proposals/` 並標明未採納；新增 `docs/ROADMAP.md`；CLAUDE.md §0.2 加文件分工表。 | 目標是「只取得這個 repo 就能開發、啟動、測試」。工作區原有 claude_healthcare／gpt_healthcare／Healthcare 三個平行資料夾，前兩者已刪除、第三者改名為 health-ref（唯讀參考）。 | Claude |
| 2026-09-05 | `health-ref`（chenni416/Healthcare）**只能借想法，不得複製程式碼**。 | 它沒有 LICENSE 檔、package.json 的 license 為空、private:true —— 著作權法預設保留所有權利；本 repo 是 Apache-2.0，複製無授權的碼進來是授權違規。CLAUDE.md §0.5 本來就這樣規定。唯二值得借的想法是通用 HealthEvent 模型（ROADMAP E3，但**不採用**其 confidence／severity／weight 欄位，那違反 §1.8）與授權/稽核的 purpose 欄位（CONSOLIDATION §4）。 | Claude |
| 2026-09-05 | 修正 CLAUDE.md 四處與 §0.1「不開分支不開 PR」矛盾的敘述（§1 標題、§0.3 稽核結果去向、§5 版本升級、§8 每個 PR），改為 commit。§0.3 的 `claude plugin add` 指令不存在（KNOWN_ISSUES #4），改寫為「已 vendor 進 .claude/skills，不需安裝」。§0.4 補 Windows 指令並註明 docker 只在有 Docker 的機器可用（#2）。 | 過期與自相矛盾的開發規則會讓每個新進來的人各自解讀。 | Claude |
| 2026-09-05 | 新增 `scripts/dev.ps1` 作為 Windows 開發入口。日常指令（setup／api／web／test／lint／check／codegen／eval／migrate／status）**保證不碰 records\ 與資料庫**；破壞性操作（init／reset／seed／clean-records）會先列出將刪除什麼並要求輸入 yes，非互動模式一律拒絕並要求 `-Force`。 | Makefile 的 db-local／reset 綁 Homebrew，只在 macOS 可用；而 `make seed` 會清空 records（KNOWN_ISSUES #31），日常操作與破壞性操作混在同一組指令裡遲早會誤刪。 | Claude |
