...
function lines(v: Record<string, unknown> | undefined | null): string[] {
  const rf = v as { hits?: { facts: string[]; action: string }[] } | null | undefined;
  if (!rf?.hits?.length) return [];
  return rf.hits.filter((h) => h.action === "notify_now").map((h) => `觀察到：${h.facts.join("；")} → 建議立即聯絡護理師`);
}

export default function ReviewPage() {
  const { thread } = useParams<{ thread: string }>();
  const tid = decodeURIComponent(thread);
  ...
  const red = lines(v.red_flags as Record<string, unknown>);
  ...
  return (
    <div className="space-y-4">
      {red.length > 0 && <RedFlagBanner lines={red} />}
      <div className="flex flex-wrap items-center gap-2">
        <h1 className="text-2xl font-medium"><span translate="no">{pid}</span> · 急症審核</h1>
            ...
            <>
              <Card variant="confirmed" title="事故檔已寫入紀錄" meta={<span translate="no">{docs.incident_file}</span>}>
                <p className="flex flex-wrap items-center gap-3 text-sm">
                  <Link href={`/nurse/incident/${pid}/${docs.incident_file}`} className="inline-flex min-h-11 items-center text-primary hover:underline">開啟事故檔 →</Link>
          ...
          {done && (
            <Card variant="confirmed" title="流程完成">
              <ul className="space-y-1 text-sm">
                ...
                <li>
                  事故檔：<Link href={`/nurse/incident/${pid}/${docs.incident_file}`} className="inline-flex min-h-11 items-center text-primary hover:underline" translate="no">{docs.incident_file}</Link>
                </li>
                <li>家屬通知：{(v.family_notification as { status: string })?.status}</li>
                <li>追蹤：{fmtDateTime((v.follow_up as { due_at: string })?.due_at)} 再問照護者一次</li>
                <li>
                  <Link href={`/record/${pid}`} className="inline-flex min-h-11 items-center text-primary hover:underline">看這個人的紀錄 →</Link>
                </li>
              </ul>
            </Card>
          )}
