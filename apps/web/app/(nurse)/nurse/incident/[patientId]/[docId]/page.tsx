...
export default function IncidentPage() {
  ...
  const { data: d, error } = useApi<IncidentFile>(`/records/${patientId}/documents/${docId}`);
  if (error) return <p role="alert" className="text-danger-ink">{error}</p>;
  if (!d) return <p className="text-ink-2">Loading…</p>;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <h1 className="text-2xl font-medium">事故檔 · <span translate="no">{patientId}</span></h1>
        ...
        <Link href={`/record/${patientId}`} className="ml-auto inline-flex min-h-11 items-center text-sm text-primary hover:underline">紀錄 →</Link>
      </div>
