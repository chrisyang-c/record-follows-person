...
function ResidentCard({ r }: { r: HomeResident }) {
  const data = r.card;
  const recorded = data.recorded_today ?? false;
  const notes = data.notes_count ?? 0;
  const red = data.session_phase === "red";
  return (
...
/** 照護者首頁：三張住民大卡（≥88px），點進去就是對話。一次呼叫 /home/caregiver。 */
export default function CaregiverHome() {
  const { data: home, error } = useApi<HomeData>("/home/caregiver");
