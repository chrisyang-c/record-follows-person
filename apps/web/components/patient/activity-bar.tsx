    ...
    <div className={cn("max-w-[85%] text-xs", className)}>
      <button
        ...
        className={cn(
          ...
          hasRed ? "border-danger text-danger-ink" : "border-ai-line text-ink-2 hover:text-ink",
        )}
      >
      ...
      {open && (
        <ol className="mt-1 space-y-1 rounded-[10px] border border-dashed border-ai-line bg-ai-fill p-2" aria-live={live ? "polite" : undefined}>
          {steps.map((e, i) => (
            <li key={i} style={{ animationDelay: `${Math.min(i, 12) * 120}ms` }} className={cn("step-in grid grid-cols-[auto_1fr_auto] gap-x-2", e.type === "red" && "text-danger-ink")}>
